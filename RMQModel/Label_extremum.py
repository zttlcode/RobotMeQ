...
def process_stock_data(allStockCode):
    ...
    for index, row in allStockCode.iterrows():
        assetList = RMQAsset.asset_generator(row['code'][3:], row['code_name'], ['30', '60', 'd'], 'stock', 1, 'A')
        data_1_filePath = (RMTTools.read_config("RMQData", "backtest_bar") +
                           "bar_" + assetList[0].assetsMarket + "_" + assetList[0].assetsCode +
                           "_" + assetList[0].barEntity.timeLevel + ".csv")
        ...
        # 保存结果为csv文件
        wave_df.to_csv("../QuantData/_signals.csv", index=False)
        break
