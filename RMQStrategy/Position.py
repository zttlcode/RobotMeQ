...
"""
人们总是过早止盈，但是却又不愿止损，他们会因为像这样的行为偏差而放弃超额回报，或遭受重大损失。行为金融理论也可以解释人们回归偏差的倾向和对不可能事件的偏好。众多投资者中存在大量的其他行为偏差，因而可以基于这一点来构建投资策略

（诸多小亏损，偶尔大收益，像买保险；损失小且频繁，风控容易）拥有正偏度收益分布的策略类型如下：·趋势跟随策略；·买入期权；·套期保值。
（诸多小收益，偶尔大亏损，像卖保险；损失大且罕见，风控不易）拥有负偏度收益分布的策略类型如下：·FX carry（外汇套利）；·做空波动率；·做市；·债券间的套利（就像LTCM公司做的那样）。

桥水做的事情是这样，我们尽量找8~10个相关性极低的资产组合在一起，就是绿色的这一块。如果相关性只有20%，你会极大地降低风险，会极大地提高夏普比率，同时亏钱的可能性也极低。王沿的讲话里只提到了资产，其实对于策略也是一样的，我们可以把策略当作特殊的资产进行配置。识别和建立不相关的交易策略组合是许多对冲基金的主要目标。人们往往希望可以测量在现有策略库中添加的新策略对现状的可预期的边际收益是多少。此外，如何通过在不同投资组合之间分配资金来优化这套新策略的夏普比率？我们在下面的讨论中解决这些问题。


基于夏普比率的资产配置
基于风险预算（Risk Budget）的资产配置

仓位管理，3种情况
1 周线高位刚下来
1.1 有卖出信号：建议忽略；1、可能在冲M顶；2、下跌中继反弹结束
1.2 有买入信号：建议忽略；博反弹50%概率能赚
2 周线跌很久，接近前低
2.1 有卖出信号：建议忽略；
2.2 有买入信号：已有日线底背离，建仓，只买不卖，信号级别越买越小，5次机会，马丁翻倍加，最后一次补仓是确认趋势反转后
3 周线涨很久，接近前高
3.1 有卖出信号：已有日线顶背离，减仓，只卖不买，信号级别越卖越小，5次机会，马丁翻倍减，最后一次卖出是确认趋势反转后
3.2 有买入信号：建议忽略；回到1.2，重新循环
"""
