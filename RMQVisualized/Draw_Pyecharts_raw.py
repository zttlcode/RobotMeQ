...
def draw_chart(assetCode):
    ...
    kline = (
        ...
        .add_yaxis(
            ...
            # 这个管的是两个交易日之间，每天累积的成交量之和 比如今天成交量55M，10天后的成交量78M，中间这10天成交量一共511
    ...
    # overlap_kline_line2 = overlap_kline_line.overlap(kline_line2)
    # overlap_kline_line3 = overlap_kline_line2.overlap(kline_line3)  不论加多少,只要横坐标一样就行
    ...
    # 最下面的柱状图和折线图
    # 跟 蜡烛图+均线一样,macd和它的两条线也要在一起
    overlap_bar_line = bar_2.overlap(line_2)

    # 最后的 Grid
    # 这是官方文档中,图表类型-组合图表-并行多图
    grid_chart = Grid()
    # pyecharts可以链式编程,也可以不链式,下面是不链式的
    ...
    # 等策略多了，就不用这个单页的了
    grid_chart.width = "90%"
    grid_chart.height = "600px"
    grid_chart.render(RMTTools.read_config("RMQVisualized", "template_folder") + assetCode + ".html")
    # 资产只用日线级别, 上面注释的连线标记,要改为买卖点,差值改为买卖点的利润

    # 等策略多了 不render直接return
    # return grid_chart
...
if __name__ == "__main__":
    ...
    assetCode = "510050"
    draw_chart(assetCode)

    # 等策略多了，就用下面的tab  增加策略,就继续加tab.add(……)
    # tab = Tab()
    # tab.add(draw_chart(), "510050-图表派策略")
    # tab.add(draw_chart(), "510050-海龟策略")
    # tab.render(RMTTools.read_config("RMQVisualized", "template_folder") + "510050.html")
